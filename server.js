...
app.post("/api/auth/signup", async function(req, res) {
  ...
  var result = await supabase
    .from("users")
    .insert([{ email: email, password_hash: hash, name: name }])
    .select()
    .single();
  if (result.error) {
...
app.post("/api/admin/activate", async function(req, res) {
  ...
  await supabase
    .from("users")
    .update({ plan: "pro", expires_at: expiresAt })
    .eq("email", email);
...
app.post("/api/admin/users", async function(req, res) {
  ...
  var result = await supabase
    .from("users")
    .select("email,name,plan,expires_at,created_at")
    .order("created_at", { ascending: false });
  res.json({ success: true, users: result.data });
});

app.listen(process.env.PORT || 3000, function() {
  console.log("SellerMate API running on port " + (process.env.PORT || 3000));
});
